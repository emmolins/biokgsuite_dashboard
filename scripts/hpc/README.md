...
#   - Aggregation + figures + bootstrap-vs-rerun table
```
...
Open the notebook locally to see all the inline figures + the bootstrap-vs-rerun comparison table.
...
| `GatedRepoError` for Gemma | License not yet approved | Either wait for Google's email, or temporarily set `GEMMA_ENABLED = False` in nb08 cell 13 and resubmit |
| Job hits the 24 hr walltime | MATRIX training was slow | Re-`sbatch` — it'll resume from cached per-rerun results, only re-doing what's missing |
| `Permission denied` writing to results/ | Repo on read-only filesystem | Don't put repo on a snapshot mount; `~` is fine |
| Want to skip MATRIX (long pole) | — | In nb08 cell 4, change `KG_NAMES = list(config['knowledge_graphs'].keys())` to exclude `'matrix'` |

## Costs and what to bring to your supervisor

The single sbatch job uses:
- 1 H100 GPU × ~2 hr (for Gemma encoding)
- 32 CPU cores × ~2-3 hr (for TransE/RotatE training × 5 reruns)
- 0 dollars (your account is presumably pre-allocated)

After the job completes, the supervisor-facing artifacts are:
- `results/figures/08_resampled_headline.png` — TransE/RotatE/Gemma per KG with rerun-empirical 95% CIs
- The bootstrap-vs-rerun table in the executed notebook — quantifies how much the bootstrap CIs were too tight
- The slide deck at `docs/llm_prompting_strategies.pptx` (separate; already done)
